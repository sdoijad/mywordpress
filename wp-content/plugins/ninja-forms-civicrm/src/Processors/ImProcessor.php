...
use NinjaForms\CiviCrmSdk\Contracts\Im as SdkObject;
use NinjaForms\CiviCrmSdk\Entities\CreateObjectResponse;

use NinjaForms\CiviCrm\Processors\ModuleProcessorAbstract;

class ImProcessor extends ModuleProcessorAbstract implements ModuleProcessor
{
    ...
    public function process(): ModuleProcessor
    {
        ...
        if (!empty($fieldMappingCollection)) {
            ...
            $existingImAddresses = $this->getExistingImAddresses($contactRecordId);

            foreach ($fieldMappingCollection as $fieldMapping) {
                try {

                    if (\in_array($fieldMapping['name'], $existingImAddresses)) {
                        continue;
                    }

                    $response = $this->createImEntry($fieldMapping, $contactRecordId);
    ...
    /**
     * Return indexed array of existing IMs for a given contact Id
     *
     * @param string $contactRecordId
     * @return array
     */
    protected function getExistingImAddresses(string $contactRecordId): array
    {
        $return = [];
        $existingRecords = $this->filterExistingRecords($contactRecordId);

        if (is_array($existingRecords)) {
            $return = \array_column($existingRecords, 'name');
        }
    ...
    protected function extractSubmissionData(): array
    {
        ...
        if (!isset($this->submissionData['im_field_map'])) {
            return $return;
        }

        foreach ($this->submissionData['im_field_map'] as $optionRepeaterValue) {
    ...
    /**
     * Create a new IM record entry linked to contact id
     *
     * @param array $fieldMapping
     * @param string $contactRecordId
     * @return CreateObjectResponse
     */
    protected function createImEntry(array $fieldMapping, $contactRecordId): CreateObjectResponse
    {
    ...
    {
        $this->recordIds['Im'][] = $response->getId();
    ...
    {
        $this->logger->warning($e->getMessage() . ' => ImProcessor.process');
    }
}
