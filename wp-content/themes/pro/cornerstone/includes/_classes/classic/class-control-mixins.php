...
class Cornerstone_Control_Mixins extends Cornerstone_Plugin_Component {
  ...
  public function setup() {
		...
		$bpService = cornerstone('Breakpoints');
		$this->cssClassMap = [

			'visibility' => [
				'xl' => $bpService->hideClass('xl'),
				'lg' => $bpService->hideClass('lg'),
				'md' => $bpService->hideClass('md'),
				'sm' => $bpService->hideClass('sm'),
				'xs' => $bpService->hideClass('xs')
			],
  ...
  /**
	...
	public function injector( $atts ) {
		...
		if ( count( $styles ) > 0 ) {
			$atts['style'] = $this->reduce_inline_styles( implode( ';', $styles ) );
		}
	...
	public function reduce_inline_styles($input) {

		$input_styles = ( is_array( $input ) ) ? $input : explode( ';', $input );

		// Break styles down into properties and values
		foreach ( $input_styles as $style ) {
			$parts = explode( ':', trim( str_replace(';', '', $style ) ) );
			if ( count( $parts ) < 2 ) {
				continue;
			}
			$output_styles[] = $style;
	...
	public function inject_margin( $inject, $atts ) {
		...
		if ( isset( $atts['margin'] ) && $atts['margin'] != '' ) {
			if ( is_array( $atts['margin'] ) ) {
				$atts['margin'] = Cornerstone_Control::simplify_dimensions( $atts['margin'] );
			}
	...
	public function inject_padding( $inject, $atts ) {
		...
		if ( isset( $atts['padding'] ) && $atts['padding'] != '' ) {
			if ( is_array( $atts['padding'] ) ) {
				$atts['padding'] = Cornerstone_Control::simplify_dimensions( $atts['padding'] );
			}
		...
		if ( isset( $atts['border_style'] ) && $atts['border_style'] != 'none' ) {
			...
			if ( isset( $atts['border_width'] ) && $atts['border_width'] != '' ) {
				if ( is_array( $atts['border_width'] ) ) {
					$atts['border_width'] = Cornerstone_Control::simplify_dimensions( $atts['border_width'] );
				}
	...
	public function inject_visibility( $inject, $atts ) {
		...
		if ( isset( $atts['visibility'] ) ) {
			...
			$visibilty_classes = $this->classMap( 'visibility', $visibilty_classes );
	...
	public function inject_text_align( $inject, $atts ) {
		...
		if ( isset( $atts['text_align'] ) && $atts['text_align'] != 'none' ) {
			$inject['classes'][] = $this->classMap( 'text_align', $atts['text_align'] );
		}
    ...
    if ( ! isset( $this->cssClassMap[ $group ] ) ) {
      trigger_error( "Cornerstone_Control_Mixins::classMap group: $group doesn't exist in class map.", E_USER_WARNING );
      return array( '' );
    }
